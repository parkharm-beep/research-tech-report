...
> [!note] 2026-05-14 소스 추가: δ-mem — 고정 크기 연상 메모리로 KV 선형 증가 문제 우회
> [[δ-mem]](arXiv:2605.12357)은 동결된 LLM 백본에 **고정 크기 상태 행렬**을 부착해 과거 정보를 압축하는 경량 메모리 메커니즘. 컨텍스트 길이와 무관한 일정 메모리 예산 → KV 캐시 선형 증가 문제를 구조적으로 회피. Delta-rule로 상태를 온라인 업데이트하고 readout이 어텐션 계산에 저랭크 보정을 적용. 백본 파인튜닝·교체 없이 MemoryAgentBench·LoCoMo에서 성능 향상 확인. KV 캐시를 대체하는 방향이 아닌 **메모리 집약 시나리오(장기 에이전트·대화)에서 컨텍스트 창을 보완**하는 접근.
